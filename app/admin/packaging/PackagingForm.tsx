...
export function PackagingForm({
  ...
  const [packagingCode, setPackagingCode] = useState(packaging?.packagingCode ?? '');
  ...
  const [minStock, setMinStock] = useState(packaging?.minStock?.toString() ?? '');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (packaging) {
        await updatePackagingMaterial(packaging.id, { packagingCode, name, supplierId: supplierId || undefined, quantity, unit, minStock });
      } else {
        await createPackagingMaterial({ packagingCode, name, supplierId: supplierId || undefined, quantity, unit, minStock });
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-4 rounded-xl bg-white border border-slate-200 p-6 relative">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Packaging code</label>
          <input type="text" value={packagingCode} onChange={(e) => setPackagingCode(e.target.value)} disabled={loading} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="number" step="0.01" value={quantity} onChange={(e) => setQuantity(e.target.value)} disabled={loading} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
        </div>
        <div>
