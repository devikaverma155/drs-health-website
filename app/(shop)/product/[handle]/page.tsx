  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 py-4 text-sm text-body-muted overflow-x-auto whitespace-nowrap" aria-label="Breadcrumb">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12 py-8 lg:py-12">
          {/* Image section */}
          <div className="w-full">
            <div className="relative w-full bg-gray-50 rounded-xl overflow-hidden shadow-sm">
              <div className="relative aspect-square w-full">
                ...
                {/* Discount badge */}
                {discountPercent > 0 && (
                  <div className="absolute top-4 right-4 w-14 h-14 bg-primary text-white rounded-full flex items-center justify-center text-sm font-bold shadow-lg">
                    -{discountPercent}%
                  </div>
                )}
              </div>
              ...
              {product.images && product.images.length > 1 && (
                <div className="flex gap-2 mt-4 overflow-x-auto pb-2">
                  {product.images.slice(0, 5).map((img, i) => (
                    <div key={i} className="relative w-16 h-16 flex-shrink-0 rounded-lg overflow-hidden border-2 border-transparent hover:border-primary transition-colors cursor-pointer bg-gray-50">
                      <Image
                        src={img.url}
          ...
          <div className="flex flex-col">
            ...
            <div className="mt-4 flex items-baseline gap-3 flex-wrap">
              ...
              <span className={`text-2xl sm:text-3xl font-bold ${hasDiscount ? 'text-accent-lime' : 'text-foreground'}`}>
            ...
            <div className="mt-4 flex items-center gap-2">
              {variant?.available !== false ? (
                <>
                  ...
                  <span className="text-sm text-accent-lime font-medium">In Stock</span>
                </>
              ) : (
                <>
            ...
            <div className="mt-6 grid grid-cols-3 gap-3">
              <div className="flex flex-col items-center text-center p-3 bg-gray-50 rounded-lg">
                <svg className="w-5 h-5 text-primary mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
