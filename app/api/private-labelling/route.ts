...
export async function POST(request: Request) {
  try {
    ...
    const fullMessage = [productInterest ? `Product Interest: ${productInterest}` : '', message || ''].filter(Boolean).join('\n');
    ...
    console.error('[api/private-labelling]', e);
    return NextResponse.json(
